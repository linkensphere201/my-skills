...
Use `scripts/start_bg_task.ps1` when possible. It creates the run directory, writes metadata, starts the command hidden, and returns the runner PID, child PID path, runtime status path, and log paths.
...
Use `scripts/stop_bg_task.ps1` with the recorded `metadata.json` path. It reads the child PID from `child-pid.txt`, stops the child first, then stops the runner PID.

```powershell
powershell -ExecutionPolicy Bypass -File skills\run-bg-task-win\scripts\stop_bg_task.ps1 `
  -MetadataPath "E:\projects\project-manager\stock-picker\.tmp\bg-tasks\<task>\metadata.json"
```
